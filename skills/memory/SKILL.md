...
```
node --experimental-sqlite --no-warnings {{SKILL_ROOT}}/scripts/session.js list
node --experimental-sqlite --no-warnings {{SKILL_ROOT}}/scripts/session.js show <session-id|name> [--json]
```
