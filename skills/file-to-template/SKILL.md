...
from these instructions: `{{SKILL_ROOT}}/scripts/templatize.js`. Nothing
about the logic lives in this file — always invoke the script rather than
re-implementing its behavior inline.

`{{SKILL_ROOT}}` is a token resolved by each agent adapter to wherever this
skill's directory actually lives for that agent (see `adapters/*/README.md`
in the repo root). It is not a literal path — don't run commands against
the string `{{SKILL_ROOT}}` itself.

## Requirements

- Node.js must be available on PATH (`node --version`). No npm packages
  are required — the script has zero dependencies.

## How it works

Two replacement modes, which combine in one run:

1. **Explicit overrides** (`--var NAME=value`) — force one exact literal
...
2. **Auto-detect** (on by default) — heuristically finds values that are
   ...
   in `"host": "localhost"`) are left alone — only values are templated.
...
```
node {{SKILL_ROOT}}/scripts/templatize.js <file> [options]
...
- Use `--var` (with or without `--no-auto`) when specific values need
  specific, predictable names — e.g. secrets, or values auto-detect can't
...
```
node {{SKILL_ROOT}}/scripts/templatize.js ./config.json --var HOST=localhost
...
- Heuristics are regex-based, not a real parser for any given file format
  — always review the report, especially for source code files where a
