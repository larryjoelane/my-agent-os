...
Implemented as standalone Node.js scripts under `../../../skills/memory/scripts/`
— always invoke them rather than re-implementing their behavior inline.
How each technique works is documented separately in
`../../../skills/memory/docs/` (`tfidf-search.md`, `hebbian-edges.md`); this file
only covers what's needed to call the scripts correctly.

`../../../skills/memory` is a token resolved by each agent adapter to wherever
...
run commands against the string `../../../skills/memory` itself.
...
```
node --experimental-sqlite --no-warnings ../../../skills/memory/scripts/recall.js "<query>" [--limit N] [--kind <kind>] [--stats]
```
...
```
node --experimental-sqlite --no-warnings ../../../skills/memory/scripts/recall.js --list [--limit N] [--kind <kind>]
```
...
```
node adapters/claude-code/condense-transcript.js | node --experimental-sqlite --no-warnings ../../../skills/memory/scripts/save-session.js --name "<name>" --themes a,b,c --summary "<summary>"
```
...
```
node --experimental-sqlite --no-warnings ../../../skills/memory/scripts/related.js <memory-id> [--limit N]
```
...
```
node --experimental-sqlite --no-warnings ../../../skills/memory/scripts/recall-store.js [--source <name>] [--tags a,b,c] "<text to remember>"
```
